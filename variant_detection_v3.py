...
from functions import compare, var_anno, dict_generator, chr_converter  # local scripts
...
while aln is not None and window_index < len(window_list):
    ...
    if -1 <= cmp <= 1:  # if intersection
        ...
        # we get the cigar_tuple and translate it
        cigar_tuple = aln.cigartuples
        tr_cigar_tuple = [(dict_tuples[pair[0]], pair[1]) for pair in cigar_tuple]
        # print(tr_cigar_tuple)
